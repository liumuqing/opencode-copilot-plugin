...
export type CopilotStandaloneOptions = {
  transform?: CopilotRequestTransform
}
...
// Standalone plugin: stubbed from ../../util/log
const log = {
  error: (message: string, meta?: unknown) => console.error("[plugin.copilot]", message, meta),
}
...
// Standalone plugin: extracted from @/session/message-v2
...
export function createCopilotAuthPlugin(options: CopilotStandaloneOptions = {}) {
  return async function CopilotAuthPlugin(input: PluginInput): Promise<Hooks> {
      ...
      async models(provider, ctx) {
        ...
        return CopilotModels.get(
          base(auth.enterpriseUrl),
          {
            ...
            "User-Agent": `opencode/${INSTALLATION_VERSION}`,
          },
          provider.models,
        ).catch((error) => {
    ...
    "chat.params": async (incoming, output) => {
      ...
      // field on tool definitions. Match upstream by opting out of AI SDK injection.
      ...
      if (
        parts?.data.parts?.some(
          (part) =>
            ...
            // Auto-compaction resumes via a marked synthetic user text part.
...
export const CopilotAuthPlugin = createCopilotAuthPlugin()
